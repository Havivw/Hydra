...
// Marauder-mode SSID lists, lifted from marauder-v1div/esp32_marauder/WiFiScan.h:339,351
const char* funny_beacon[] = {
  "Abraham Linksys", "Benjamin FrankLAN", "Dora the Internet Explorer",
  "FBI Surveillance Van 4", "Get Off My LAN", "Loading...",
  "Martin Router King", "404 Wi-Fi Unavailable", "Test Wi-Fi Please Ignore",
  "This LAN is My LAN", "Titanic Syncing", "Winternet is Coming"
};
const int funnyCount = sizeof(funny_beacon) / sizeof(funny_beacon[0]);

const char* rick_roll[] = {
...
const int rickCount = sizeof(rick_roll) / sizeof(rick_roll[0]);

// Charset for MODE_RANDOM (Marauder-style true-random SSIDs)
const char* randCharset =
  "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789 ";
const int randCharsetLen = 63;

// Mode state — cycle with DOWN button. MODE_CIFER = original 37-item list.
enum SpamMode { MODE_CIFER = 0, MODE_FUNNY = 1, MODE_RICKROLL = 2, MODE_RANDOM = 3, MODE_COUNT = 4 };
int spamMode = MODE_CIFER;
const char* modeNames[MODE_COUNT] = { "Cifer", "Funny", "Rick ", "Rand " };
...
void handleDownButton() {
  spamMode = (spamMode + 1) % MODE_COUNT;
}
...
void spammer() {
  ...
  // Pick SSID based on current mode
  String randomSSID;
  if (spamMode == MODE_CIFER) {
    randomSSID = ssidList[random(ssidCount)];
  } else if (spamMode == MODE_FUNNY) {
    randomSSID = String(funny_beacon[random(funnyCount)]);
  } else if (spamMode == MODE_RICKROLL) {
    randomSSID = String(rick_roll[random(rickCount)]);
  } else {  // MODE_RANDOM
    int len = random(8, 25);
    char buf[33];
    for (int i = 0; i < len; i++) buf[i] = randCharset[random(randCharsetLen)];
    buf[len] = '\0';
    randomSSID = String(buf);
  }

  int ssidLength = randomSSID.length();
  if (ssidLength > 32) ssidLength = 32;  // 802.11 SSID cap
  ...
  // Lay down post-SSID tags (supported rates + DSSS channel) immediately after
  // the SSID body. Original cifertech code assumed SSID len = 6 and hardcoded
  // frame length 57; this version derives both from actual SSID length so
  // Marauder-mode longer names (up to 32 chars) transmit cleanly.
  static const uint8_t postSSID[13] = {
    0x01, 0x08, 0x82, 0x84, 0x8b, 0x96, 0x24, 0x30, 0x48, 0x6c,  // supported rates
    0x03, 0x01, 0x00                                              // DSSS, channel filled below
  };
  for (int i = 0; i < 13; i++) {
    packet[38 + ssidLength + i] = postSSID[i];
  }
  packet[38 + ssidLength + 12] = spamchannel;  // channel byte of DSSS element

  int frameLen = 38 + ssidLength + 13;
  esp_wifi_80211_tx(WIFI_IF_AP, packet, frameLen, false);
...
void beaconSpamSetup() {
  ...
  tft.setCursor(2, 50 + y_offset);
  tft.print("[!] Press [DOWN] to cycle mode");
  tft.setCursor(2, 70 + y_offset);
  tft.print("[!] Press [LEFT/RIGHT] for channel");
...
void beaconSpamLoop() {
  ...
  bool btnDownPress = !pcf.digitalRead(BTN_DOWN);
...
void deauthdetectSetup() {
  // Kill any zombie tasks from a prior entry. The dispatcher's BTN_SELECT
  // exit bypasses this feature's own stopScan/exitMode cleanup path, leaving
  // the three xTaskCreate'd workers running. Re-entering would spawn a
  // second set, the handles would be overwritten, and the old tasks would
  // keep mutating shared state until the firmware crashed.
  if (wifiScanTaskHandle)   { vTaskDelete(wifiScanTaskHandle);   wifiScanTaskHandle   = NULL; }
  if (uiTaskHandle)         { vTaskDelete(uiTaskHandle);         uiTaskHandle         = NULL; }
  if (statusBarTaskHandle)  { vTaskDelete(statusBarTaskHandle);  statusBarTaskHandle  = NULL; }
  if (tftSemaphore)         { vSemaphoreDelete(tftSemaphore);    tftSemaphore         = NULL; }
  stopScan = false;
  exitMode = false;
    ...
    if (attack_running && current_time - last_channel_check > 15000) { // Every 15s
        ...
        if (checkApChannel(selectedAp.bssid, &new_channel)) {
            if (new_channel != selectedChannel) {
                ...
                wifi_config_t ap_config = {0};
                strncpy((char*)ap_config.ap.ssid, "Hydra", sizeof(ap_config.ap.ssid));
                ap_config.ap.ssid_len = strlen("Hydra");
                strncpy((char*)ap_config.ap.password, "deauth123", sizeof(ap_config.ap.password));
                ap_config.ap.authmode = WIFI_AUTH_WPA2_PSK;
                ap_config.ap.ssid_hidden = 0;
                ap_config.ap.max_connection = 4;
...
// ---- Shared WiFi feature cleanup --------------------------------------------
//
// Called from the menu dispatcher (displayMenu) when leaving a WiFi feature.
// Stops promiscuous-mode RX so the previous feature's callback doesn't keep
// firing into its statics while a new feature initializes, and tears down
// AP/STA state. We do NOT call esp_wifi_deinit — that has been observed to
// destabilize the BT+WiFi coexistence partner; subsequent features expect
// the controller to still be alive and just reconfigure mode.
//
// Mirror of bleFeatureCleanup() in bluetooth.cpp.
void wifiFeatureCleanup() {
  // esp_wifi_set_promiscuous(false) is the only call that's safe when WiFi
  // is uninitialized — it just returns ESP_ERR_WIFI_NOT_INIT. We tolerate
  // that silently; the goal is to make this a no-op on cold boot.
  esp_wifi_set_promiscuous(false);
  esp_wifi_set_promiscuous_rx_cb(NULL);

  // softAPdisconnect(true) tears down DHCP server + clears client list.
  // WiFi.disconnect(true) clears stored creds + disconnects. Both no-op
  // if WiFi was never used.
  WiFi.softAPdisconnect(true);
  WiFi.disconnect(true);

  delay(20);  // let the WiFi task drain pending events before next feature inits
}
