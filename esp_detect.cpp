/*
 * Espressif Detector — Hydra port. See header.
 */
...
// Known Espressif OUI prefixes. The single OUI in Marauder's Assets.h is
// fc:f5:c4. Espressif has several public OUI allocations — these are
// well-known examples (verify against IEEE registry if anything's missed).
...
static const int ESP_OUI_COUNT = sizeof(ESP_OUIS) / sizeof(ESP_OUIS[0]);
...
static bool IRAM_ATTR isEspOui(const uint8_t* mac) {
  for (int i = 0; i < ESP_OUI_COUNT; i++) {
    if (mac[0] == ESP_OUIS[i][0] && mac[1] == ESP_OUIS[i][1] && mac[2] == ESP_OUIS[i][2]) return true;
  }
  return false;
}
...
static void IRAM_ATTR wifiSniffer(void* buf, wifi_promiscuous_pkt_type_t type) {
  ...
  if (pktLen < 38) return;
  if (payload[0] != 0x80) return;  // beacons only

  if (!isEspOui(payload + 10)) return;

  uint8_t ssidLen = payload[37];
  if (ssidLen > 32) ssidLen = 32;
  char essid[33];
  if (ssidLen == 0) {
    strncpy(essid, "(hidden)", sizeof(essid));
  } else {
...
static void pushDispRow(const AlertRecord& r) {
  ...
  snprintf(row.text, sizeof(row.text),
           ...
           r.mac[0], r.mac[1], r.mac[2], r.essid, (int)r.channel);
}
...
static void redrawTopStatus() {
  ...
  tft.setCursor(80, 24); tft.print("ESP:"); tft.print(totalHits);
...
void espDetectSetup() {
  ...
  tft.setCursor(2, 45);
  tft.print("[!] Espressif Device Detector");
  ...
  esp_wifi_set_channel(currentChannel, WIFI_SECOND_CHAN_NONE);
  lastHop = millis();
  Serial.println("[ESP] sniffer started, hopping 1..11");
...
void espDetectLoop() {
  ...
  while (drained < 4 && popAlert(r)) {
    ...
    totalHits++;
    pushDispRow(r);
    Serial.printf("[ESP] %02x:%02x:%02x:%02x:%02x:%02x essid=%s ch=%d rssi=%d\n",
