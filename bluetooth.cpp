...
#include <esp_wifi.h>   // for esp_wifi_stop/deinit in BLE scan setup
...
void toggleAdvertising() {
  ...
  if (!isAdvertising) {
    ...
    if (attack_state == 1) {
      ...
      // addServiceUUID moved to spooferSetup — see note there.
...
void spooferSetup() {
  ...
  // Add the service UUID once in setup — addServiceUUID pushes onto an
  // unbounded BLEUUID vector. Previously called in toggleAdvertising,
  // which leaked one entry per attack toggle.
...
void sourappleSetup() {
  ...
  // Add the service UUID once in setup — addServiceUUID pushes onto an
  // internal std::vector<BLEUUID> that the library never trims. Previously
  // this lived in the loop and grew by one entry per ~40ms iteration
  // (~25Hz), exhausting the heap after ~10 minutes of running the attack
  // and crashing the BT stack. The custom adv data set per iteration via
  // setAdvertisementData takes precedence on-air anyway, so the per-loop
  // call was a no-op for the actual broadcast.
...
void sourappleLoop() {
  ...
  // addServiceUUID moved to sourappleSetup — calling it in the loop leaked
  // memory at ~25Hz into an unbounded internal vector.
...
// Local snapshot of scan results — populated during the scan via a
// per-device callback. We can't trust the BLEScanResults map after the
// scan ends because its BLEAdvertisedDevice* pointers can be invalidated
// by the BT stack on the next scan/clear, which is the crash path the
...
static BleDevSnap bleSnap[BLE_SCAN_MAX];
static int bleSnapCount = 0;

class BleCaptureCb : public BLEAdvertisedDeviceCallbacks {
  void onResult(BLEAdvertisedDevice dev) override {
    if (bleSnapCount >= BLE_SCAN_MAX) return;
    BleDevSnap& s = bleSnap[bleSnapCount];
    // Bind the std::string returned by getName() to a local so its lifetime
    // extends past the c_str() read. Storing c_str() of an unbound temporary
    // is a use-after-free — that was a recurring crash in the BLE scanner.
    std::string nameStr = dev.getName();
    if (!nameStr.empty()) {
      strncpy(s.name, nameStr.c_str(), sizeof(s.name) - 1);
      s.name[sizeof(s.name) - 1] = '\0';
    } else {
      strcpy(s.name, "Unknown");
    }
    std::string addrStr = dev.getAddress().toString();
    strncpy(s.addr, addrStr.c_str(), sizeof(s.addr) - 1);
    s.addr[sizeof(s.addr) - 1] = '\0';
    s.rssi = dev.getRSSI();
    bleSnapCount++;
  }
};
static BleCaptureCb bleCb;
...
void startBLEScan() {
  if (!bleScan) {
    Serial.println("[BleScan] startBLEScan called with null bleScan");
    return;
  }
  displayScanning();
  isScanning = true;
  ...
  // Reset our own snapshot before scan so the callback populates fresh.
  bleSnapCount = 0;

  Serial.println("[BleScan] starting 3s scan...");
  bleScan->clearResults();
  // The return value of start() is the unsafe map-of-pointers — we ignore
  // it. Our BleCaptureCb populated bleSnap[] during the scan instead.
  bleScan->start(3, false);
  bleScan->stop();
  bleScan->clearResults();   // drop the pointers we don't use
  isScanning = false;
  screenNeedsUpdate = true;
  Serial.printf("[BleScan] scan done, %d devices\n", bleSnapCount);
}
...
void updateBLEList() {
  ...
  int deviceCount = bleSnapCount;  // local snapshot, safe to read
  if (deviceCount <= 0) {
    if (fullScreenUpdate) {
  ...
  if (listStartIndex >= deviceCount) listStartIndex = deviceCount - 1;
  ...
  for (int i = 0; i < 14; i++) {
    ...
    String deviceName = bleSnap[index].name;
...
void displayBLEDetails() {
  ...
  if (currentIndex < 0 || currentIndex >= bleSnapCount) {
    tft.setCursor(10, 60);
    tft.setTextColor(TFT_RED);
    tft.print("(no device selected)");
    return;
  }
  const BleDevSnap& snap = bleSnap[currentIndex];
  String deviceName = snap.name;
  String address = snap.addr;
  int rssi = snap.rssi;
  int txPower = 0;
  ...
  // (We dropped Service UUID / Manufacturer Data / Service Data — they
  // came from the unsafe BLEAdvertisedDevice* pointer that crashes us.
  // The basics — name, MAC, RSSI — survive in our snapshot.)
}
...
void bleScanSetup() {
  ...
  // Stop any prior BLE scan/advertise from the last feature, then any WiFi
  // promisc state left over (Marauder scan, AP scan, ESPPwn, etc). Don't
  // fully deinit — that has destroyed the BT controller's coexistence
  // partner on some retries and made the BLE init that follows panic.
  // Just stopping scan/advertise + promisc off + stop is enough.
  bleFeatureCleanup();
  esp_wifi_set_promiscuous(false);
  esp_wifi_stop();
  delay(50);

  // BLEDevice::init also serves as re-init if a previous feature deinit'd
  // the BT controller. Wrap in a defensive try-equivalent: check getScan()
  // returns non-null before we hand it to the scan loop.
  BLEDevice::init("");
  delay(50);
  bleScan = BLEDevice::getScan();
  if (!bleScan) {
    Serial.println("[BleScan] getScan() returned null — BLE init failed");
    tft.setCursor(10, 60);
    tft.setTextColor(TFT_RED);
    tft.print("BLE init failed — reboot");
    return;
  }
  bleScan->setActiveScan(true);
  bleScan->setInterval(100);
  bleScan->setWindow(99);
  // Per-device callback fires while scan is running, captures into our own
  // array. Avoids the BLEScanResults map-of-pointers lifetime trap.
  bleScan->setAdvertisedDeviceCallbacks(&bleCb, false, true);
...
private:
  // (Removed `TFT_eSPI tft;` member — it shadowed the global `tft` and was
  // never init()-ed, so any draw call routed through it wrote to an
  // unconfigured SPI peripheral. All `tft.X()` calls in this class now
  // resolve to the global declared in bleconfig.h.)
...
public:
  void setup() {
    ...
    // Stop any prior BLE scan/advertise + WiFi promisc state from the last
    // feature. WiFi+BT share the radio; promisc-mode leftovers panic the
    // BT stack the moment BLEDevice::init starts the controller.
    bleFeatureCleanup();
    esp_wifi_set_promiscuous(false);
    esp_wifi_stop();
    delay(50);

    // BLEDevice::init internally calls esp_bt_controller_mem_release with
    // ESP_BT_MODE_CLASSIC_BT, which FREES the classic-BT memory. Any
    // subsequent esp_bt_controller_init(BTDM) or esp_bt_gap_start_discovery
    // call after this is poking freed memory — that's the crash path the
    // original code took. We drop the classic-BT scan attempt and run as a
    // BLE-only sniffer with suspicious-activity tracking on top.
    BLEDevice::init("");
    delay(50);
    pBLEScan = BLEDevice::getScan();
    if (!pBLEScan) {
      addLine("BLE init failed - reboot", RED, true, MessageType::ALERT);
      return;
    }
    // Allocate the callback object exactly once across the firmware's lifetime.
    // Previously this setup() did `new AdvertisedDeviceCallbacks(*this)` on every
    // entry, leaking ~16B per re-entry — after a few menu visits the heap was
    // exhausted and BLEDevice::init started failing.
    static AdvertisedDeviceCallbacks* cb = nullptr;
    if (!cb) cb = new AdvertisedDeviceCallbacks(*this);
    pBLEScan->setAdvertisedDeviceCallbacks(cb);
    pBLEScan->setActiveScan(true);
    pBLEScan->setInterval(100);
    pBLEScan->setWindow(99);

    addLine("BLE Sniffer Ready (BLE only)", DARK_GRAY, true, MessageType::STATUS);
    startBLEScan();
  }
  ...
  void loop() {
    ...
    if (scanning && now - lastScanTime >= SCAN_INTERVAL) {
      // Classic-BT scan path removed — BLEDevice::init freed that memory.
      // We loop BLE scans only. Each scan re-feeds the callback which
      // populates devices[] for suspicious-activity tracking.
      if (pBLEScan) {
        pBLEScan->stop();
...
// ---- Shared BLE feature cleanup ---------------------------------------------
//
// Called from the menu dispatcher when leaving a BT feature. Stops any active
// scan or advertise so the next feature's setup doesn't race with stale
// callbacks (the root cause of the cross-feature crashes we kept hitting).
//
// We intentionally do NOT call BLEDevice::deinit() — past attempts to fully
// deinit destroyed the controller's coexistence partner and panic'd the next
// BLEDevice::init(). The Arduino BLE singletons (getScan/getAdvertising) are
// idempotent across init()s, so stopping radio activity is sufficient.
void bleFeatureCleanup() {
  // Guard: getScan/getAdvertising dereference internal singletons that are
  // only valid after BLEDevice::init has run at least once. If the BT
  // controller isn't enabled, there's nothing in flight to stop.
  ...
  delay(20);  // let the controller settle before the next feature's init
}
